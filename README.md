# 💳 Credit Card Fraud Detection (Kafka + Spark Streaming + FastAPI)
...
├── api/
│   ├── app.py                # FastAPI inference service (/predict)
│   ├── fraud_model.pkl        # Trained model (generated by training script)
│   ├── Dockerfile
│   └── requirements.txt
├── model/
│   ├── train_model.py         # Train XGBoost + export fraud_model.pkl
│   └── requirements.txt
├── producer/
│   ├── producer.py            # Kafka producer that streams CSV rows
│   ├── Dockerfile
│   └── requirements.txt
├── processor/
│   ├── spark_job.py           # Spark Structured Streaming consumer + API caller + SQLite writer
│   ├── Dockerfile
│   └── requirements.txt
└── docker-compose.yml         # Brings up Zookeeper, Kafka, API, Producer, Spark Processor
```
...
- Python 3.10+
- You’ll need Java for Spark if running `processor/` locally (Docker handles this already)

---

## 📦 Dataset Setup

This project uses the common Kaggle dataset **“Credit Card Fraud Detection”**.
...
## 🧠 Train / Retrain the Model (optional)
...
## 🗃️ Where are predictions stored?
...
## 🧪 Running Components Individually (local, non-Docker)
...
If running locally, you’ll need to update `bootstrap_servers` in `producer.py` (e.g., `localhost:9092`) and ensure Kafka is accessible.
